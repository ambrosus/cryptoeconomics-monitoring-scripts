...
import {
  ...
  sortedResolvedChallengesCountByTier
...
describe('Challenge stats', () => {
  ...
  describe('sortedResolvedChallengesCountByTier', () => {
    const exampleChallengesCount = _({
      '0x1': 1,
      '0x2': 2,
      '0x3': 1,
      '0x4': 1
    });
    const exampleStakes = _({
      '0x1': '100000000000000000000000',
      '0x2': '200000000000000000000000',
      '0x3': '99000000000000000000000',
      '0x4': '100000000000000000000001'
    });
    const expectedResult = [['0x2', 2], ['0x4', 1], ['0x1', 1], ['0x3', 1]];

    it('sorts correctly', async () => {
      expect(sortedResolvedChallengesCountByTier(exampleChallengesCount, exampleStakes).value()).to.deep.equal(expectedResult);
    });
  });

  describe('formatDistribution', () => {
    const exampleTierStats = _({
      100: 20,
      200: 15,
      300: 17
    });

    it('returns total resolutions count normalized to 100 sorted by stake decreasing', async () => {
      expect(formatDistribution(exampleTierStats)).to.equal('33/29/38');
    });
  });
});
